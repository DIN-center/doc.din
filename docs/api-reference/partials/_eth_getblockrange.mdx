...
- `start block`: (string) _[required]_ - Hexadecimal block number, or one of the string tags
...
- `end block`: (string) _[required]_ - Hexadecimal block number, or one of the string tags
  `latest`, `earliest`, `pending`, `safe`, or `finalized`.
  See the [default block parameter](https://ethereum.org/en/developers/docs/apis/json-rpc/#default-block).
...
Array of block objects. Each block object contains:
...
- `mixHash`: 32 bytes. The hash of the generated proof-of-work.
...
```bash
wscat -c <ENDPOINT> -x '{"jsonrpc": "2.0", "method": "eth_getBlockRange", "params": ["0x1", "0x2", false], "id": 1}'
```
