...
DIN Provider uses the EigenLayer Application to become an Operator
DIN AVS is notified of applying DIN Provider to an Operator Set
DIN Provider is reviewed by Watchers to be added to identity
DIN App is updated with a status for DIN Provider from the DIN AVS process
...
DIN Provider connects wallet and uses SIWE EOA identity for staking
Interface for staking X amount to DIN Token Deposit
DIN AVS is notified of the new DIN Provider identity to initialize Watcher confirmation and approval
DIN App is updated with a status for DIN Provider
...
Watchers informed on newly added DIN Endpoints
Watchers perform endpoint testing
Watchers update testing data to AVS and Node explorer metadata
DIN Router / Proxy updated with DIN Endpoints
...
Watchers ping nodes to live update node health
DIN Router stores reputation on successfully completed relays
DIN Payment module updated on relays
...
DIN AVS checks DIN Provider reputation for threshold
Warnings and slashing is invoked by DIN AVS
